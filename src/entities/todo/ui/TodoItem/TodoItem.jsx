...
function TodoItem({ className = "", id, title, isDone }) {
  const {
  ...
  return (
    ...
    >
      <input
        ...
        type="checkbox"
        checked={isDone}
        onChange={(event) => toggleTaskCompleted(id, event.target.checked)}
      />
      <label className={`${styles.label} visually-hidden`} htmlFor={id}>
        {title}
      </label>
      <RouterLink to={`tasks/${id}`} className={styles.link} aria-label="Страница сведений о задаче">
